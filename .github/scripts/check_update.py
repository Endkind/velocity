...
def main():
    all_papermc_api_velocity_versions = PaperMCAPIUtils.get_all_versions(
        PaperMCAPIProject.VELOCITY
    )
    ...
    if all_papermc_api_velocity_versions.is_err():
        print(
            f"Failed to fetch PaperMC versions: {all_papermc_api_velocity_versions.unwrap_err()}"
        )
        raise SystemExit(1)

    all_papermc_api_velocity_versions = all_papermc_api_velocity_versions.unwrap()

    for papermc_api_velocity_version in all_papermc_api_velocity_versions:
        if papermc_api_velocity_version in all_local_disabled_versions:
            current_status = VersionSupportStatus.DISABLED
        elif papermc_api_velocity_version in all_local_versions:
            current_status = VersionSupportStatus.SUPPORTED
        else:
            current_status = VersionSupportStatus.UNSUPPORTED

        version_support_status[papermc_api_velocity_version] = current_status

        if current_status == VersionSupportStatus.UNSUPPORTED:
            issue_title = f"New Velocity version {papermc_api_velocity_version}"
            if issue_title not in open_gh_issue_titles:
                result = GitHubAPIUtils.create_issue(
                    title=issue_title,
                    body=f"Version {papermc_api_velocity_version} is not supported by this repository yet. Please add support for this version.",
                    assignees=["Endkind"],
                    labels=["update"],
                )

                if result.is_ok():
                    print(f"Issue created for version {papermc_api_velocity_version}")
                else:
                    print(
                        f"Failed to create issue for version {papermc_api_velocity_version}: {result.unwrap_err()}"
                    )
